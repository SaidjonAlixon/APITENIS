    ...
    def do_POST(self):
        try:
            ...
            if current_time > booking_end_datetime:
                response = {
                    'status': 'expired',
                    'message': 'Ticket muddati tugagan - ishlatib bo\'lmaydi',
                    'expired_at': booking_end_datetime.isoformat()
                }
                self.wfile.write(json.dumps(response).encode('utf-8'))
                return
            
            # Ticket ni ishlatilgan deb belgilash
            cursor.execute("""
                UPDATE tickets 
                SET consumed_at = %s 
                WHERE booking_id = (SELECT id FROM bookings WHERE ticket_id = %s)
            ...
            response = {
                'status': 'consumed',
                'message': 'Ticket muvaffaqiyatli ishlatildi',
                'ticket_id': ticket_id,
